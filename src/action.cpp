...
void Actions() {
	...
	int BinDown = 0;
	int BinUp = 0;
	//IN for loop we calculating up nad down summaries
	for(int i = 0; i < 4; i++){
		for(int j = 0; j < i; j++){
			BinDown += matrix[i][j];
		}
...
//BinMatrixDown/Up is a array, owing to which Binary function is working. They are binary function argument
...
//For loop is changing the matrix is up and down number to 0 or 1

for(int i = 0, e = 0, f = 0; i < 4; i++){
	for(int j = 0; j < i; j++){

//If is important, because if our 0's or 1's are less than the numbers, which must be changed, program will not be corect
...
//Finaly we are outputing the matrix
